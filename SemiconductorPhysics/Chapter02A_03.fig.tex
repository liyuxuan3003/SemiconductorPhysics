...
\draw[dis] (P1) -- node[left] {$\delt{E_\text{D}}$} (P1|-A1);

\foreach \x in {2,3,4,5}
{
    \draw[-latex] (P\x) ++(0,0.2) node[circ] {} ++(0,0.2) -- ++(0,1) coordinate (Q\x); 
}

\foreach \x in {6,7}
{
    \draw (Q2-|P\x) node[circ] {};
    \draw (P\x) ++(0,-0.4) node[point] {$+$};
}

\path (P7) ++(0.25,0) node[right] {$E_\text{D}$};
