...
\end{Equation}
这里我们注意到两件事，首先\xrefpeq{3}说明，\empx{电子的平均漂移速度和电场强度成正比}，因此我们可以将两者的比值定义成一个常数。其次，这个常数的表达式同时也由\xrefpeq{3}给出了。
...
\begin{BoxFormula}[漂移电流密度]
    半导体中，漂移电流密度满足
    \begin{Equation}
        J=J_\text{n}+J_\text{p}=nq\mu_n\Emf+pq\mu_p\Emf
    \end{Equation}
...
\begin{BoxFormula}[半导体的迁移率与电导率]*
    ...
    \begin{Equation}
        \sigma=\sigma_\text{n}+\sigma_\text{p}=nq\mu_\text{n}+pq\mu_\text{p}
    \end{Equation}
\end{BoxFormula}